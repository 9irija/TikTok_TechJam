"""Standalone check for agent/model_zoo/deepfm_din.py (DIN-style attention
over user history) -- same methodology this pass already used for
LightGBM: get a real, honest single-seed number before deciding whether
full agent/experiment.py pipeline integration is worth the engineering
investment. Not wired into the Model Zoo registry / Research Map yet.

Trains with EXACTLY deepfm_regularized's hyperparameters (k=16,
hidden=[128,64], lr=0.001, l2=1e-4, epochs=20, patience=5) so the only
variable is the added DIN attention block -- same isolation discipline
`deepfm_mtl_v1` itself followed against `deepfm_regularized`.

Usage: python tools/check_sequence_model.py [--seq_len 10] [--data_dir ...]
"""
...
from agent.model_zoo.deepfm_din import build  # noqa: E402
...
def main() -> int:
    ...
    model = build(dim, n_fields=Xtr.shape[1], k=16, hidden=[128, 64], lr=0.001, l2=1e-4,
                  video_vocab_size=video_vocab_size, seed=args.seed)
    ...
    for ep in range(1, args.epochs + 1):
        ...
        losses = [model.seq_step(Xtr[idx[i:i + bs]], ytr[idx[i:i + bs]],
    ...
    print(f"\ndeepfm_regularized (parent, for comparison): valid primary=0.6035, test primary=0.5977")
    return 0
