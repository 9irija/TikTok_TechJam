...
_This track does not require a video as part of the submission requirement, but a ~3min video is recommended if it helps you to explain your solution effectively. Without a video, a detailed report is highly encouraged._
...
\[4\] H. Zhao, G. Cai, J. Zhu, Z. Dong, J. Xu, and J.-R. Wen, "Counteracting Duration Bias in Video Recommendation via Counterfactual Watch Time," KDD 2024. Code: [https://github.com/hyz20/CWM](https://github.com/hyz20/CWM) — optional advanced reference, not the official baseline. Its contribution is a censored-regression loss on watch time (a completed play means the true watch time was truncated by video length, so a one-sided loss is used instead of squared error). Note it ships no Recall implementation, reports nDCG@1/3/5 on a rebuilt `long_view2` label, and requires `torch==1.6.0`.
